...
| `storeContextsToLocal`                                   | Boolean | 否       | 是否把用户自定义数据缓存到本地 localstorage, 例如： `setUser`, `addGlobalContext` api 添加的自定义数据 |
| `sendContentTypeByJson`                                   | Boolean | 否       | 是否以 json 的方式发送数据，即 write/rum 请求发送 `content-type: application/json` |
